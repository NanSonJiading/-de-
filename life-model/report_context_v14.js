(()=>{
  ...
  function normalizeVersion(){
    ...
    body.querySelectorAll('.rchip').forEach(x=>{if(/V1\.2|V1\.3|V1\.5/.test(x.textContent||''))x.textContent='基础生命模型 · V1.4';});
    body.querySelectorAll('*').forEach(x=>{if(x.children.length===0&&/V1\.2|V1\.3|V1\.5/.test(x.textContent||''))x.textContent=x.textContent.replace(/V1\.2|V1\.3|V1\.5/g,'V1.4');});
  }
  function inject(){
    ...
    card.innerHTML=`<h3>场景差异｜你的反应并不是在所有情况下都一样</h3><div class="sub">“看具体情况”不是无效回答，它说明某些模式可能只会在特定条件下出现。</div>
      <div class="block"><h4>最可能改变你反应的条件</h4>${ranked.length?`<div class="v14-tags">${ranked.map(([k,n])=>`<span>${esc(labels[k]||k)} · ${n}</span>`).join('')}</div><p>这意味着报告不能只写“你通常怎样”，还需要写清楚：<b>在什么关系、风险和现实条件下，这个反应才更容易出现。</b></p>`:'<p>本次没有记录明确的场景边界。</p>'}</div>
      ${reasons.length?`<div class="block"><h4>现有选项没有完全覆盖你的地方</h4><ul>${reasons.map(([k,n])=>`<li>${esc(reasonLabels[k]||k)}：${n} 次</li>`).join('')}</ul><p>这些记录不会被强行映射成人格字段，而会用于后续题目修订和结果边界判断。</p></div>`:''}
      <details class="research"><summary>查看场景差异记录</summary><div class="pilot-table">${a.records.map(r=>`<div class="pilot-row"><b>${esc(r.qid)}</b><span>${r.type==='boundary'?esc((r.variables||[]).map(v=>labels[v]||v).join(' · ')):esc(reasonLabels[r.reason]||r.reason)}</span><small>${esc(r.measurementGoal||'')}</small></div>`).join('')}</div></details>
      <div class="racts"><button class="secondary" id="v14ExportContext">导出场景记录</button></div>`;
