(()=>{
  ...
  function inject(){
    if(!$('report')?.classList.contains('active')||document.getElementById('v14ContextCard'))return;
    ...
    card.innerHTML=`<h3>你的反应并不是在所有场景里都一样</h3><div class="sub">V1.4 开始把“看具体情况”当成正式信息，而不是无效答案。</div>
      <div class="block"><h4>最可能改变你反应的条件</h4>${ranked.length?`<div class="v14-tags">${ranked.map(([k,n])=>`<span>${esc(labels[k]||k)} · ${n}</span>`).join('')}</div><p>这意味着未来的生命模型不能只写“你通常怎样”，还需要写清楚：<b>在什么关系、风险和现实条件下，这个模式才会启动。</b></p>`:'<p>本次没有记录明确的场景边界。</p>'}</div>
